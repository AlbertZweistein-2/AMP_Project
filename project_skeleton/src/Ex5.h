...
static inline uint64_t pack(void *ptr, uint16_t stamp) {
    return ((uint64_t)stamp << STAMP_SHIFT) | ((uint64_t)ptr & PTR_MASK);
}

static inline void* unpack_ptr(uint64_t packed) {
    return (void*)(packed & PTR_MASK);
}
