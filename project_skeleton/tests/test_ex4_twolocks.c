...
static void test_two_locks() {
    ...
    #pragma omp parallel num_threads(num_threads)
    {
        ...
        if(thread_id == 0){
            ...
            assert(q.head->next->data == 10);
            node_t* first_node = q.head->next;
            lock_peterson(&q.plock, 1); // Pretend to dequeue
            // Wait a few cycles to ensure thread 1 is waiting
            assert(q.plock.flag[1] == true); 
            for(volatile int i = 0; i < 100000; i++);
            assert(q.head->next == first_node); // Ensure no other enqueue happened
            assert(q.head->next->data == 10);
            unlock_peterson(&q.plock, 1);
            assert(q.plock.flag[1] == false);

        }
        else if(thread_id == 1){
            while(!q.plock.flag[1]);
            enq(42, &q, thread_id);
            assert(q.head->next->next != NULL);
            assert(q.head->next->data == 10);
            assert(q.head->next->next->data == 42);
        }
    }
    ...
    #pragma omp parallel num_threads(num_threads)
    {
        ...
        if(thread_id == 0){
            value_t v;
            deq(&v, &q, thread_id, NULL);
            assert(v == 10);
            node_t* second_node = q.head->next;
            lock_peterson(&q.plock, 0); // Pretend to dequeue
            // Wait a few cycles to ensure thread 1 is waiting
            assert(q.plock.flag[0] == true); 
            for(volatile int i = 0; i < 100000; i++);
            assert(q.head->next == second_node); // Ensure no other dequeue happened
            assert(q.head->next->data == 42);
            unlock_peterson(&q.plock, 0);
            assert(q.plock.flag[0] == false);
        }
        else if(thread_id == 1){
            while(!q.plock.flag[0]);
            value_t v;
            deq(&v, &q, thread_id, NULL);
            assert(v == 42);
            assert(q.head->next == NULL);
